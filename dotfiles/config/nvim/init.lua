...
Installation:
ln -s ~/p/voyt_dotfiles/dotfiles/nvim ~/.config/nvim
...
-- The <Cmd> pseudokey begins a "command mapping", which executes the command directly without changing modes.
vim.keymap.set({ 'n', 'v' }, '<leader>q', '<Cmd>q<CR>', { desc = '[Q]uit window' })
vim.keymap.set({ 'n', 'v' }, '<leader>w', '<Cmd>w<CR>', { desc = '[W]rite the buffer to the current file' })
...
require('lazy').setup({
  ...
  { -- Useful plugin to show you pending keybinds.
    ...
    opts = {
      ...
      icons = {
        ...
        -- Nerd Font: set icons.keys to an empty table
        keys = {},
      },
      ...
      spec = {
        ...
        { '<leader>c', group = '[C]ustom', mode = { 'n', 'v' } },
      },
    },
  },
  ...
  }, -- end of telescope plugin config
  ...
  -----------------------------------------------------------------------------
  {
    ...
    dependencies = {
      ...
      -- NOTE: `opts = {}` is the same as calling `require('mason').setup({})`
      {
        ...
        version = '2.2.1',
        opts = {
          -- to use system installed rust-analyzer, we need system PATH:
          PATH = 'append',
        },
      },
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      -- TODO: check out: mini.git
    end,
  },
  -----------------------------------------------------------------------------
  { -- Highlight, edit, and navigate code
    -- This plugin requires: sudo pacman -S tree-sitter-cli
  ...
  -----------------------------------------------------------------------------
  {
    -- Draws vertical line for each indentation
...
vim.keymap.set('n', '<leader>tc', function() vim.cmd.HighlightColors 'Toggle' end, { desc = 'Toggle highlight colors' })
...
vim.keymap.set('n', '<leader>rn', function() vim.diagnostic.jump { count = 1, float = true } end, { desc = 'Rust - Show Next Diagnostic' })
...
vim.lsp.config('rust_analyzer', {
  settings = {
    ['rust-analyzer'] = {
      ...
      -- diagnostics = {},
      check = { command = 'clippy' }, -- use clipy instead of cargo check to show all problems
    },
  },
})
